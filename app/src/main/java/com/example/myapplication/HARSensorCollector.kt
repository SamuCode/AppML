...
class HARSensorCollector(
            ...
            Sensor.TYPE_ACCELEROMETER -> {
                if (event.timestamp - lastAccTs >= targetIntervalNs * 0.8) {
                    for (i in 0..2) {
                        ...
                        if (accBuf[i].size > UCIHARFeatureExtractor.WINDOW_SIZE * 2)
                            accBuf[i].pollFirst()
                    }
            ...
            Sensor.TYPE_GYROSCOPE -> {
                if (event.timestamp - lastGyroTs >= targetIntervalNs * 0.8) {
                    for (i in 0..2) {
                        ...
                        if (gyroBuf[i].size > UCIHARFeatureExtractor.WINDOW_SIZE * 2)
                            gyroBuf[i].pollFirst()
                    }
    ...
    private fun checkWindowReady() {
        val ws = UCIHARFeatureExtractor.WINDOW_SIZE
        val overlap = ws / 2
        if (accBuf[0].size >= ws && gyroBuf[0].size >= ws) {
            samplesProcessed++
            if (samplesProcessed % overlap == 0) {
